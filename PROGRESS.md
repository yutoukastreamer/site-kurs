...
> Последнее обновление: 2026-04-10 (VideoSection v3 — scroll-zoom до fullscreen без zoom-out, текст скрывается только по клику play, режим плеера с ползунком перемотки и крестиком закрытия)
...
- [x] **VideoSection в стиле fluid.glass** — 200vh sticky, текст «Система в действии» исчезает при зуме, кнопка «Смотреть видео» рестартит со звуком
...
- [x] **VideoSection v2 — акцентный фон + боковые тексты-накладки (desktop)** — фон секции теперь окрашен акцентным цветом машины (синий для бульдозера, янтарь для экскаватора, зелёный для грейдера), без градиентов. Заголовок «Система в действии» слева, описание справа; оба наезжают на рамку видео и плавно уходят при скролле. Тексты исчезают при нажатии «Смотреть видео». Кнопка не пересекается.
- [x] **VideoSection — пауза и скролл-пауза** — в watch-режиме добавлена кнопка пауза/воспроизведение рядом с close. Видео автоматически ставится на паузу, когда пользователь скроллит далеко вверх или вниз от секции (IntersectionObserver threshold 0.3).
- [x] **VideoSection mobile — статичный горизонтальный плеер** — убрана cinematic-анимация. Заголовок и описание расположены над видео. Плеер aspect-video с кнопками play (центр), pause и fullscreen (правый нижний угол). Кастомный fullscreen-оверлей с X-кнопкой для возврата, блокирующий скролл страницы.
- [x] **Header скрывается на подвале (desktop)** — IntersectionObserver на `#main-footer` в Header.jsx: когда подвал попадает в viewport, шапка уезжает вверх (`lg:-translate-y-full lg:opacity-0 lg:pointer-events-none`). На мобиле шапка остаётся.
- [x] **VideoSection v3 — scroll-zoom без zoom-out + режим плеера** — Scroll-zoom теперь достигает fullscreen (scale=1) на 60% прогресса и удерживается (без обратного zoom-out). Тексты «Система в действии» (слева) и описание (справа) остаются видимыми при скролле и исчезают ТОЛЬКО при нажатии «Смотреть видео». Watch-режим: крестик закрытия вверху справа, нижняя панель управления с кнопкой play/pause + текущее время + ползунок перемотки (range input) + общая длительность. Body scroll lock при активном плеере. useMotionValue + animate() для плавных переходов scale/borderRadius при входе/выходе из watch-режима.
