...
|---|---|---|---|
| 1 | Шапка | `Header.jsx` | Логотип ГСИ слева, навигация: Бульдозер, Экскаватор, Грейдер, Галерея, Где купить. Телефон + CTA "Получить предложение". Мобильное бургер-меню. Фиксированная, blur при скролле. **На главной до скролла — светлая тема** (белый текст/логотип/кнопка поверх видео). |
| 2 | Hero | `HeroHome.jsx` | **Fullscreen с оверлеем**. Видео на весь экран, тёмный градиент поверх, заголовок "Инновационные отечественные 3D системы нивелирования", слоган, 2 кнопки (Получить предложение + Смотреть продукцию), логотипы Курс и Сделано в России (белые, brightness-0 invert). |
...
| 5 | Где купить | `WhereToBuy.jsx` | Кликабельный список филиалов (Москва, СПб, Казань, Новосибирск, Краснодар) с подсветкой активного + Яндекс Карта (конструктор iframe). |
...
- [x] **VideoSection v2 — акцентный фон + боковые тексты-накладки (desktop)** — фон секции окрашен акцентным цветом машины, заголовок слева, описание справа
- [x] **VideoSection — пауза и скролл-пауза** — в watch-режиме кнопка пауза/воспроизведение, авто-пауза при скролле вне секции
- [x] **VideoSection mobile — статичный горизонтальный плеер** — aspect-video, fullscreen-оверлей с X-кнопкой
- [x] **Header скрывается на подвале (desktop)** — IntersectionObserver на `#main-footer`
- [x] **VideoSection v3 — scroll-zoom без zoom-out + режим плеера** — scale 0.55→1 (60%), панель с ползунком перемотки, body scroll lock
- [x] **Заголовок hero «Инновационные»** — заменено «Первые» → «Инновационные» в HeroHome.jsx
- [x] **Шапка: светлая тема на главной** — на homepage до скролла белый текст, белый логотип ГСИ (brightness-0 invert), белая кнопка CTA, белый бургер. При скролле — стандартная тёмная тема
- [x] **Hero: логотипы Курс/Россия белые** — brightness-0 invert, opacity-90/70
- [x] **Подвал: логотип КУРС белый** — brightness-0 invert opacity-70
- [x] **Грейдер: изображение в схеме крупнее** — ringMachineScale: 0.75 (было 0.6 default)
...
- [x] ~~API-ключ Яндекс Карт~~ — подключена карта через конструктор (iframe)
...
- [x] Подключить Яндекс Карту — iframe конструктора
